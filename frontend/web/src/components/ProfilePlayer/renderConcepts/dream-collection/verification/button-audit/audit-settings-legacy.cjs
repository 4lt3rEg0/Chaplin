...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});const reports=[],errors=[];try{
 ...
 selected=id;await page.goto('http://127.0.0.1:5173/settings');await page.getByRole('button',{name:'Ir a '+({'aqua-flow':'Aqua Flow','aero-amp':'AeroAmp','bubblegum-gloss':'Y2K Bubblegum Gloss'}[id]),exact:true}).click();const p=page.locator('[data-profile-player]').first();await p.getByRole('button',{name:'Reproducir',exact:true}).waitFor();
 ...
 fs.writeFileSync(path.join(output,'button-audit-settings-legacy.json'),JSON.stringify({passed:true,reports},null,2));
