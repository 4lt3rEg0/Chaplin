...
(async()=>{const browser=await chromium.launch({channel:'msedge',headless:true});const reports=[],errors=[];try{
 ...
 for(const j of jobs){
  ...
  await seek.fill('9.6');await expect.poll(async()=>(await audioState()).time,{timeout:5000}).toBeLessThan(2);await expect(player.locator('.dc-title')).toHaveText('Moonlit Tides');await expect.poll(async()=>(await audioState()).paused).toBe(false);
  await player.getByRole('button',{name:'Modo de reproducción: Repetir canción',exact:true}).click();
  await player.getByRole('button',{name:'Canción siguiente'}).click();await expect(player.locator('.dc-title')).toHaveText('Ocean Dreamer');await expect(seek).toBeEnabled();await seek.fill('9.6');await expect(player.locator('.dc-title')).toHaveText('Moonlit Tides',{timeout:5000});
  await player.getByRole('button',{name:'Modo de reproducción: Repetir lista',exact:true}).click();await expect(seek).toBeEnabled();await seek.fill('9.6');await expect(player.locator('.dc-title')).toHaveText('Ocean Dreamer',{timeout:5000});
 ...
 fs.writeFileSync(path.join(output,'button-audit-all.json'),JSON.stringify({passed:true,skins:reports,scope:"All twelve rendered skins",note:'Isolated API/auth fixtures. Real HTMLAudioElement and Web Audio decode PCM; user account not accessed.'},null,2));
